...
class DomainexpirationValidationException implements Exception {
  final List<String> errors;

  DomainexpirationValidationException(this.errors);

  @override
  String toString() => 'DomainexpirationValidationException: ${errors.join("; ")}';
}
...
/// Domain Expiration API client.
///
/// For more information, visit: https://domainexpiration.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [domain] (required) - The domain to check the expiration date of [format: date]
class DomainexpirationClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'domain': ValidationRule(type: 'string', required: true, format: 'date'),
  };

  DomainexpirationClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/domainexpiration',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DomainexpirationValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DomainexpirationValidationException(errors);
    }
  }

  /// Execute a request to the Domain Expiration API.
  ///
  /// Parameters are validated before sending the request.
  Future<DomainexpirationResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DomainexpirationException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DomainexpirationResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DomainexpirationException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DomainexpirationException('Resource not found');
      } else {
        throw DomainexpirationException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DomainexpirationException) rethrow;
      throw DomainexpirationException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Domain Expiration API client.
class DomainexpirationException implements Exception {
  final String message;

  DomainexpirationException(this.message);

  @override
  String toString() => 'DomainexpirationException: $message';
}
